 ...
 * - `Board.grid: Array<Array<SushiTile?>>` — cells may be `null` for empty
 ...
 * @property grid        row-major 2D array. `grid[row][col]` may be `null`
 ...
 *
 * Implementation note: `Array<Array<SushiTile?>>` uses generic arrays.
 * `Board.equals` is reference-based for the `grid` field (Kotlin's
 * `Array.equals` is identity, not structural). For structural equality use
 * `grid.contentDeepEquals(other.grid)`. The Engine layer compares positions
 * element-by-element, so this is not a blocker.
 */
data class Board(
    val size: Int = 7,
...
// ============================================================================
// 7. GamePhase — REMOVED in v3 (2026-06-20 18:42, subagent-S, T-VM-001)
// ============================================================================
//
// `GamePhase` was a `sealed class` with `IDLE` / `ANIMATING` / `GAME_OVER`
// (defined in T-CORE-008 by subagent-A). It is now an `enum class` with
// `IDLE` / `PLAYING` / `PAUSED` / `GAME_OVER`, defined in
// `engine/GameState.kt` next to `GameState` (both are UI snapshots, not
// pure-engine data models). The PAUSED state supports activity-lifecycle
// `onPause()` / `onResume()` calls from the ViewModel.
//
// This section is intentionally left as a pointer comment so that subsequent
// line numbers don't shift. See GameState.kt for the new definition.

// ============================================================================
// Manual test entry (per T-CORE-008 acceptance criteria)
// ============================================================================

// main for manual test
// Verifies the T-CORE-008 invariants:
//   - Board 默认 size=7
//   - 6 种 SushiType
//   - 4 个 Direction
//   - 2 种 MatchAxis (added in v2)
fun main() {
    val board = Board()
    check(board.size == 7) { "Board default size should be 7, was ${board.size}" }

    val sushiCount = SushiType.entries.size
    check(sushiCount == 6) { "SushiType should have 6 entries, was $sushiCount" }

    val dirCount = Direction.entries.size
    check(dirCount == 4) { "Direction should have 4 entries, was $dirCount" }

    val axisCount = MatchAxis.entries.size
    check(axisCount == 2) { "MatchAxis should have 2 entries, was $axisCount" }

    // Sanity: defaults on the lock flags
    check(!board.swapLock) { "Board default swapLock should be false" }
    check(!board.cascadeLock) { "Board default cascadeLock should be false" }

    // Sanity: default grid is size×size
    check(board.grid.size == 7) { "Board default grid should be 7×7" }
    check(board.grid.all { row -> row.size == 7 }) { "Board default grid rows should be 7" }

    // GamePhase v3: moved to GameState.kt as enum class with 4 values
    // (IDLE / PLAYING / PAUSED / GAME_OVER). Verified there.

    println("Models.kt manual test passed:")
    println("  - Board default size = ${board.size}")
    println("  - SushiType entries = $sushiCount (${SushiType.entries.joinToString()})")
    println("  - Direction entries = $dirCount (${Direction.entries.joinToString()})")
    println("  - MatchAxis entries = $axisCount (${MatchAxis.entries.joinToString()})")
    println("  - Board default grid = ${board.grid.size}×${board.grid[0].size} (all cells null)")
    println("  - GamePhase: see engine/GameState.kt (moved in v3)")
}