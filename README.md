...
> 受经典 MTK 功能机游戏《魔法寿司》启发的 Android 三消游戏。
> 玩法规则在原版基础上由作者按个人偏好重新定义 —— **不是复刻或移植**。

**版本：** v1.0.0
**平台：** Android 8.0+ (API 26+)
**技术栈：** Kotlin + Jetpack Compose + ViewModel + StateFlow + Canvas
**授权：** 保留所有权利，仅供学习研究，禁止商用（见 [NOTICE.md](./NOTICE.md)）
...
4. 每次成功消除 → 倒计时**重置回 60 秒**（不是累加，与本次消除数量无关）
5. 倒计时结束 → 显示得分 + 最高分

**计分：** `10 × 消除个数 × 长度倍数 × 连锁倍数`
（长度倍数 3/4/5/6+ 连 = `1.0`/`1.5`/`2.0`/`3.0`；
连锁倍数第 1/2/3/4+ 次 = `1.0`/`1.5`/`2.0`/`2.5`）
...
- 代码：作者原创，仅供学习研究，**禁止商用**
- 寿司图片与音效：源自 MTK 固件资源，**无公开授权**，不得二次分发
