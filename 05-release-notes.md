...
- **消除后倒计时立即重置，不再在低时间线上误判失败**

  时间剩 1~2 秒时拼出消除，此前会在消除动画播放途中直接判定游戏结束 ——
  分数没加、时间也没回来。现在判定消除的那一刻就把时间补满，并在整个
  结算动画期间冻结计时，动画结束后才继续走表。

  连锁消除也受益：此前动画播放期间倒计时照常流逝，连锁越长、被吃掉的
  奖励时间越多；现在无论连锁几轮，拿到的都是完整的 60 秒。
...
- **暂停按钮加圆形背景与描边**。此前是一个白色图标孤零零压在深棕底上，
  缺少「这是可点的东西」的边界；尺寸从 20dp 一路调到 32dp 都没解决 ——
  那时「显眼」全靠图标自身的白色面积去顶。改为 48dp 圆形容器（半透明深棕底
  + 2dp 琥珀描边，与提示条同一套配色），图标反而缩回 Material 标准 24dp，
  由整个圆承担存在感。

  ⚠️ 触摸区始终是 48dp，从未改变 —— 20dp 时看着小但可点范围已达无障碍下限。
  中途试过 44dp 圆，但显式传给 `IconButton` 会把默认 48dp 触摸区一起缩掉，
  为 4dp 观感牺牲可点范围不值。

- **倒计时改为屏幕正中**。原来数字既不居中也不靠边。根因不是位置参数写错，
  而是 `TimerDisplay` 拿的是 `weight(1f)` —— 它居中于「暂停按钮右侧的剩余
  宽度」而非屏幕，被推得偏右约半个按钮宽。改用 `Box` 让计时器相对整个屏幕
  宽度居中，左侧控件叠在上面 `align(CenterStart)` 钉住，不再参与宽度计算。
